...
const CartPage = ({ username: propsUsername, setOrderItems }) => {
  const [username, setUsername] = useState(
    propsUsername || sessionStorage.getItem('username')
  );
  ...
  const handleBuyNow = async (product) => {
    ...
    const placeOrderAndRemoveFromCart = async () => {
      try {
        const res = await PlaceOrder(username, product.productId);
        setOrderItems((prev) => [...prev, product]);

        // Remove from cart
        await RemoveFromCart(username, product.productId);
        setCartItems((prev) =>
          prev.filter((item) => item.productId !== product.productId)
        );
    ...
    if (alreadyOrdered) {
      Modal.confirm({
        ...
        cancelText: 'Cancel',
        onOk: placeOrderAndRemoveFromCart,
      });
      return;
    }

    await placeOrderAndRemoveFromCart();
  };
